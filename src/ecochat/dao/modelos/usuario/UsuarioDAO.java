...
public class UsuarioDAO {
	 ...
	 * 
	 * @param id
	 *            código do usuário a ser alterado
	 * @param sobrenome
	 *            novo sobrenome a ser atribuído ao usuário
	 */
	public static int updateSobrenome(int id, String sobrenome) {
		...
		try {
			...
			qtdeRegAtualizados = session.createQuery(hqlUpdate).setString("novoSobrenome", sobrenome)
					.setInteger("antigoId", id).executeUpdate();
			trns.commit();
		} catch (RuntimeException e) {
			if (trns != null)
				trns.rollback();
	...
	/**
	 * Recupera o nome completo de um usuário, pesquisando o banco com base no
	 * seu primeiro nome
	 * 
	 * @param primeiroNome
	 *            nome parcial do usuário, sem o seu sobrenome.
	 */
	public static void getNomeCompleto(String primeiroNome) {
		...
		try {
			...
			List<Usuario> users = session.createQuery("from Usuario as u where u.nome = :firstName")
